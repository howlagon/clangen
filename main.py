...
async def main():
    import scripts.housekeeping.platformwrapper as web
    ...
    if web.is_web:
        ...
        web.eval("""
        FS.mount(window.IDBFS, {'root': '.'}, '/saves')
        FS.syncfs(true, (err) => {
        ...
        if platform.window.localStorage.getItem("hasMigrated") is None: # pylint: disable=no-member
            ...
            for i in range(platform.window.localStorage.length): # pylint: disable=no-member
                ...
                if key in ['hasMigrated', '/', 'currentclan', 'clanlist.txt', '__test__']:
                    continue
                os.makedirs(os.path.dirname(f"/saves/saves/{key}"), exist_ok=True)
                with open(f"/saves/saves/{key}", "w") as f:
                    f.write(value)
